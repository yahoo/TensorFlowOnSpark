...
class TFNodeContext:
  ...
  def __init__(self, worker_num, job_name, task_index, cluster_spec, mgr):
    ...
    def _reserve(iter):
        ...
        if TFSparkNode.mgr is not None and str(TFSparkNode.mgr.get('state')) != "'stopped'":
            ...
            if tensorboard and job_name == 'worker' and task_index == 0:
                ...
                if os.environ.contains('PYSPARK_PYTHON'):
                  # user-specified Python (typically --archives Python.zip)
                  ...
                  # system-installed Python & tensorboard in PATH
...
def start(fn, tf_args, cluster_info, background):
    """
    ...
    def _mapfn(iter):
        ...
        ctx = TFNodeContext(worker_num, job_name, task_index, spec, mgr)
