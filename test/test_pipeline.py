...
class PipelineTest(test.SparkTest):
  ...
  def setUpClass(cls):
    ...
    cls.train_examples = [
        (cls.features[i].tolist(), [cls.labels[i].item()]) for i in range(1000)]
  ...
  def test_namespace(self):
    ...
    # from dictionary
    d = {'string': 'foo', 'integer': 1, 'float': 3.14,
  ...
  def test_TFParams(self):
    ...
    expected_args = Namespace(
        {'a': 1, 'b': 2, 'batch_size': 10, 'steps': 100})
  ...
  def test_spark_checkpoint(self):
    ...
    trainDF = self.spark.createDataFrame(
        self.train_examples, ['col1', 'col2'])
  ...
  def test_spark_saved_model(self):
    ...
    # take first/only result
    preds = model.transform(testDF).head()
    # unpack pred scalar from tensor
    pred = preds.cout1[0]
    # unpack squared pred from tensor
    squared_pred = preds.cout2[0]
  ...
  def get_function(self, name):
          ...
          if sv.is_chief:
            if args.model_dir:
              ...
              TFNode.export_saved_model(
                  sess, export_dir=args.export_dir, tag_set='test_tag', signatures=signatures)
            else:
    ...
    def _tf_export(args):
      ...
      with tf.Session() as sess:
        ...
        assert ckpt and ckpt.model_checkpoint_path, "Invalid model checkpoint path: {}".format(
            args.model_dir)
