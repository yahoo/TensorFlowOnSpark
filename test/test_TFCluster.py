...
class TFClusterTest(test.SparkTest):
  ...
  def test_basic_tf(self):
    ...
    cluster = TFCluster.run(
        self.sc, _map_fun, tf_args=args, num_executors=self.num_workers, num_ps=0)
    cluster.shutdown()
  ...
  def test_inputmode_spark(self):
    ...
    cluster = TFCluster.run(self.sc, _map_fun, tf_args={
    }, num_executors=self.num_workers, num_ps=0, input_mode=TFCluster.InputMode.SPARK)
